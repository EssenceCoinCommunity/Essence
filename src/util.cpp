...
const char * const BITCOIN_CONF_FILENAME = "essence.conf";
const char * const BITCOIN_PID_FILENAME = "essenced.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "essence" is a composite category enabling all Essence-related debug output
            if(ptrCategory->count(string("essence"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EssenceCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EssenceCore
    // Mac: ~/Library/Application Support/EssenceCore
    // Unix: ~/.essencecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EssenceCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EssenceCore";
#else
    // Unix
    return pathRet / ".essencecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty essence.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override essence.conf
