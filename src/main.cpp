...
#if defined(NDEBUG)
# error "Essence Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
   ...
   if (nPrevHeight < 2000) {
        ...
        nSubsidy = nPrevHeight/1000*nPrevHeight/1000*2 * COIN;
    }else{
        nSubsidy = 315 * COIN;
    }
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // Structure of the MN rewards
    if(nHeight > nMNPIBlock)                  ret += blockValue / 100;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 100;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 100;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 100;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 4)) ret += blockValue / 100;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 5)) ret += blockValue / 100;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 6)) ret += blockValue / 100;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 7)) ret += blockValue / 100;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 8)) ret += blockValue / 100;
    return ret;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Essence : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ESS): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ESS): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Essence : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ESS): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ESS): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Essence Related Inventory Messages
