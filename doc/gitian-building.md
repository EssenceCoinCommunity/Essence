...
*Setup instructions for a Gitian build of Essence Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Essence
...
- [Building Essence Core](#building-essence-core)
...
Clone the git repositories for Essence Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/essencecoin/essence
```
...
in the Essence Core repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Essence Core
----------------

To build Essence Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Essence Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/essence/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/essencecoin/essence
...
For example:
```bash
URL=https://github.com/crowning-/essence.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit essence=${COMMIT} --url essence=${URL} ../essence/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit essence=${COMMIT} --url essence=${URL} ../essence/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit essence=${COMMIT} --url essence=${URL} ../essence/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the essence git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../essence/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/essence.git
SIGPATH=/some/root/path/essence-detached-sigs.git

./bin/gbuild --url essence=${BTCPATH},signature=${SIGPATH} ../essence/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/essence-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/essence-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/essence-osx-build.assert
```
...
[essence/gitian.sigs](https://github.com/essencecoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
